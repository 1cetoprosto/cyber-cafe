...
- MVVM
- основне сховище: Firebase Firestore
- Realm: legacy-кеш / історичний шар, який не є цільовим source of truth для нових фіч
- AutoLayout
- верстка кодом
...
Скрипт запускає `bundle exec fastlane ios prod` з кореня репозиторію.
Для авторизації використовується `App Store Connect API key`, тому `app-specific password` для цього сценарію не потрібен.
...
У корені цього репозиторію є файл **`AGENTS.md`** (також відомий як AI Guide).  
Він призначений **не для розробників безпосередньо**, а для AI-асистентів (ChatGPT, Copilot, Cursor, Claude, Xcode MCP тощо), які можуть використовуватись під час розробки.  
Назва `AGENTS.md` відповідає рекомендації Apple для автопідхоплення Xcode MCP та Codex/Claude CLI.
...
Докладно — дивись файл [`AGENTS.md`](./AGENTS.md).
