...
class SignInController: UIViewController {
    ...
    private func setupBio() {
        ...
        if canUseBio && UserSession.current.rememberUser && (UserSession.current.useBioAuth ?? false) && authLink == nil {
            switch context.biometryType {
                case .touchID:
                    bioAuthButton.setImage(R.image.touchId()!.tint(color: UIColor.Button.title), for: .normal)
                    bioAuthButton.setImage(R.image.touchId()!.tint(color: UIColor.Button.title.withAlphaComponent(0.5)), for: .highlighted)
                case .faceID:
                    bioAuthButton.setImage(R.image.faceId()!.tint(color: UIColor.Button.title), for: .normal)
                    bioAuthButton.setImage(R.image.faceId()!.tint(color: UIColor.Button.title.withAlphaComponent(0.5)), for: .highlighted)
                case .none: break
                case .opticID: break
    ...
    @objc private func checkByBio() {
        ...
        if canUseBio && authError == nil {
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: R.string.auth.bioAuthReason()) {[weak self] (success, error) in
    ...
    private func handleModel(_ action: AuthModel.AuthModelAction) {
        ...
        case .success(let setPassword):
            ...
            let context = LAContext()
            let canUseBio = context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &authError)
            let askUser = UserSession.current.useBioAuth == nil || UserSession.current.userEmail != UserSession.current.useBioAuthUser
            if setPassword {
                let askBio = canUseBio && authError == nil && UserSession.current.rememberUser && askUser
                navigationController?.pushViewController(SetPasswordController(askBio ? context : nil), animated: true)
    ...
    private func setupUI() {
        ...
        let buttonsStack = UIStackView.VStack([loginButton, bioAuthButton.wrapH(), signUpButton, forgotButton], spacing: 20)
    ...
    @objc private func loginAction(_ sender: UIButton) {
        ...
        guard let path = Bundle.main.path(forResource: "Config", ofType: "plist"),
            ...
            fatalError("Помилка при зчитуванні конфігурації Firebase")
        }
        ...
        model.signIn(email: emailField.text!,
    ...
    func showConfirmEmail(_ completion: @escaping (String) -> Void) {
        let alertVC = UIAlertController(title: R.string.auth.signIn(),
        ...
        alertVC.addAction(UIAlertAction(title: R.string.global.cancel(), style: .destructive, handler: {[weak self] (_) in
            self?.waitEmailConfirmation = false
        }))
        
        alertVC.addAction(UIAlertAction(title: R.string.auth.signIn(), style: .default, handler: {[weak self] action in
            self?.waitEmailConfirmation = false
            if let email = alertVC.textFields?.first?.text, email.isValid(regex: .email) {
                completion(email)
            } else {
                self?.showAlert(R.string.global.error(), body: R.string.global.emailInvalid(), action: {
