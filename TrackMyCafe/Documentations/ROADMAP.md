...
- Не змішувати в цьому релізі reporting UI.

### `1.0.10` — Inventory Behavior (Track Ingredients toggle)

- Issue: `#156`
- Додає глобальний `Track Ingredients` (on/off), який керує автоматичним списанням/відновленням складу при продажі та поверненні.
- За замовчуванням `Off`, тому існуючим користувачам нічого не ламається.
- Рецепти і склад залишаються доступними незалежно від toggle; toggle лише забороняє автоматичні рухи складу.
- Це патч-реліз без помітних UI/UX змін для користувачів у фінансовому режимі.

### `1.1.0` — Dashboard/Reports Foundation
