...
class DomainDatabaseService: DomainDB {
    ...
    func deleteIngredient(model: IngredientModel, completion: @escaping (Bool) -> Void) {
        ...
        if isOnline {
            ...
            guard let realmModel = RealmDatabaseService.shared.fetchIngredient(byId: model.id)
            else {
                completion(false)
                return
            }
    ...
    func fetchIngredient(byId id: String, completion: @escaping (IngredientModel?) -> Void) {
        if let realmModel = RealmDatabaseService.shared.fetchIngredient(byId: id) {
            completion(IngredientModel(realmModel: realmModel))
        } else {
            completion(nil)
        }
    }
    ...
    func fetchPurchases(completion: @escaping ([PurchaseModel]) -> Void) {
        ...
        if isOnline {
            ...
            let models = RealmDatabaseService.shared.fetchPurchases().map {
                PurchaseModel(realmModel: $0)
            }
            completion(models)
        }
    }
    ...
    func savePurchase(model: PurchaseModel, completion: @escaping (Bool) -> Void) {
        ...
        if isOnline {
            ...
            let realmModel = RealmPurchaseModel(dataModel: model)
            RealmDatabaseService.shared.save(model: realmModel)
            completion(true)
        }
    }
    ...
    func fetchRecipe(
        ...
        if isOnline {
            ...
            let models = RealmDatabaseService.shared.fetchRecipe(forProductId: productId).map {
                RecipeItemModel(realmModel: $0)
            }
            completion(models)
        }
    }
    ...
    func updateProduct(
        ...
        if isOnline {
            ...
            RealmDatabaseService.shared.updateProduct(
                model: updatedModel, date: date, name: name, quantity: quantity, price: price,
                sum: sum)
        }
    }
    ...
    func fetchOrders(completion: @escaping ([OrderModel]) -> Void) {
        ...
        if isOnline {
            ...
            let orders = RealmDatabaseService.shared.fetchOrders().map {
                OrderModel(realmModel: $0)
            }
            completion(orders)
        }
    }
    ...
    func updateProductsPrice(model: ProductsPriceModel, name: String, price: Double) {
        ...
        if isOnline {
            ...
            guard let updatedModel = RealmDatabaseService.shared.fetchProductPrice(byId: model.id)
            else {
                return
            }
    ...
    func saveProductsPrice(productPrice: ProductsPriceModel, completion: @escaping (Bool) -> Void) {
        ...
        if isOnline {
            ...
            RealmDatabaseService.shared.save(
                model: RealmProductsPriceModel(dataModel: productPrice))
            completion(true)
        }
    }
    ...
    func updateCost(model: CostModel, date: Date, name: String, sum: Double) {
        ...
        if isOnline {
            ...
            guard let updatedModel = RealmDatabaseService.shared.fetchCost(byId: model.id) else {
                return
            }
            RealmDatabaseService.shared.updateCost(
                model: updatedModel, date: date, name: name, sum: sum)
        }
    }
    ...
    func fetchCosts(completion: @escaping ([CostModel]) -> Void) {
        ...
        if isOnline {
            FirestoreDatabaseService.shared.read(
                ...
                case .failure(let error):
                    self.logger.error(
                        "Error fetching costs from Firestore: \(error.localizedDescription)")
                    completion([])
                }
            }
        } else {
    ...
    func transferProductOfOrdersToRealm(completion: @escaping () -> Void) {
        self.transferCollectionToRealm(
            ...
            domainModelInit: { firModel in
                ...
                guard let orderId = firModel.orderId, let newOrderId = self.orderIdMap[orderId]
                else {
                    self.logger.error(
                        "Missing order ID mapping for \(String(describing: firModel.orderId))")
                    return domainModel
                }
