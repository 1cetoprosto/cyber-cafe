...
class RealmDatabaseService: RealmDB {
    ...
    func save<T: Object>(model object: T) {
        executeWrite {
            // Fix: Check object schema for primary key instead of generic type T
    ...
    func updateProduct(
        model: RealmProductModel, date: Date, name: String, quantity: Int, price: Double,
        sum: Double
    ) {
        executeWrite {
            model.date = date
    ...
    func fetchPurchases() -> [RealmPurchaseModel] {
        logger.log("Fetched all purchases")
        return Array(
            localRealm.objects(RealmPurchaseModel.self).sorted(byKeyPath: "date", ascending: false))
    }

    // MARK: - Work with Recipes

    func fetchRecipe(forProductId productId: String) -> [RealmRecipeItemModel] {
        logger.log("Fetched recipe for product id: \(productId, privacy: .public)")
        guard let product = fetchObjectById(ofType: RealmProductsPriceModel.self, id: productId)
        else {
            return []
        }
