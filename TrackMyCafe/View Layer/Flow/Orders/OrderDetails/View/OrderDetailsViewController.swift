...
class OrderDetailsViewController: UIViewController, UITextFieldDelegate {
    ...
    let typeTextfield: UITextField = {
        let textField = UITextField()
        textField.textAlignment = .left
        textField.placeholder = "Choose type"
    ...
    fileprivate func setData() {
        if viewModel == nil {
            ...
            viewModel = OrderDetailsViewModel(model: OrderModel(id: "",
                                                                    date: Date(),
                                                                    type: "",
                                                                    sum: 0.0,
                                                                    cash: 0.0,
    ...
    func saveModels(completion: @escaping () -> Void) {
        ...
        if viewModel.isNewModel {
            ...
            viewModel.updateOrders(id: viewModel.id,
                                  date: datePicker.date,
                                  type: typeTextfield.text,
                                  cash: cashTextfield.text,
                                  card: cardTextfield.text,
...
extension OrderDetailsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: idOrderCell, for: indexPath) as? OrderTableViewCell
        ...
        tableViewCell.productStepper.addTarget(self, action: #selector(self.stepperValueChanged(_:)), for: .valueChanged)
...
extension OrderDetailsViewController {
    func setConstraints() {
        ...
        let cashCardStackView = UIStackView(arrangedSubviews: [cashStackView, cardStackView],
                                        axis: .vertical,
                                        spacing: 5,
                                        distribution: .equalSpacing)
        view.addSubview(cashCardStackView)

        let moneyStackView = UIStackView(arrangedSubviews: [cashCardStackView, orderLabel],
                                         axis: .horizontal,
                                         spacing: 10,
