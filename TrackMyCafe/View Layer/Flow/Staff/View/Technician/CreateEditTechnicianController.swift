...
class CreateEditTechnicianController: UIViewController, Loggable {
  ...
  private func setupUI() {
    ...
    let techModSwitch = makeSwitch(
      label: moderType
        ? R.string.global.addTechnicianPrivilege() : R.string.global.addModerationPrivilege(),
  ...
  @objc private func saveTechnician() {
    ...
    if let image = newImage {
      let fileName = [technician.firstName, technician.lastName, Date().interval.string].compactMap
      { $0 }.joined(separator: "_")
      StorageService.uploadAvatar(image, name: fileName) {
        ...
        if success {
          ...
          RequestManager.shared.log(
            ...
            description: R.string.global.uploadedNewTechnicianImageAt(filePath.emptyIfNil))
          ...
          if let masterUserRef = UserSession.current.masterUserRef {
            if self.technician.role == .technician || self.technician.role == .techMod {
              let techRole = RoleConfig(
                ...
                dataRef: UserSession.current.masterUserRef, userRef: userRef.documentID,
  ...
  var onChange: ((String) -> Void)? {
    ...
    set {
      objc_setAssociatedObject(
        self, &AssociatedKeys.onChangeKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    }
  }
}
