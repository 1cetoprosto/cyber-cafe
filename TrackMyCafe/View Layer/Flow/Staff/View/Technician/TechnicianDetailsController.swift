...
class TechnicianDetailsController: UIViewController {
    ...
    private func setupNavigationBar() {
        navigationItem.rightBarButtonItems = [
            UIBarButtonItem(image: UIImage(named: "edit"), style: .plain, target: self, action: #selector(editTechnician)),
            UIBarButtonItem(image: UIImage(named: "delete"), style: .plain, target: self, action: #selector(deleteTechnician))
        ]
    }
    
    private func setupNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(updateForm), name: .techniciansInfoReload, object: nil)
    }
    
    @objc private func updateForm() {
        if let updatedItem = RequestManager.shared.technicians.first(where: { $0.firebaseRef == technician.firebaseRef }) {
    ...
    private func buildForm() {
        ...
        let canSendEmail = technician.email.isValid(regex: .email)
    ...
    private func buildHeaderSection() {
        ...
        if self.fromMod {
            privileges = technician.role == .techMod ? "Has Technician Privilege" : "Does Not Have Technician Privilege"
        } else {
    ...
    private func buildAllowCalculationSection() {
        guard technician.role == .technician || technician.role == .techMod || technician.role == .administrator else {
            return
        }
        ...
        switchLabel.text = "Allow Calculation"
    ...
    private func buildNoteSection() {
        ...
        noteTextView.text = "Technician's note goes here."
    ...
    private func buildActionButtonsSection() {
        ...
        allOrdersButton.setTitle("All Orders", for: .normal)
        ...
        pricesButton.setTitle("Prices", for: .normal)
        ...
        printButton.setTitle("Print Prices", for: .normal)
    ...
    private func buildResendInviteSection() {
        ...
        resendButton.setTitle("Resend Invite", for: .normal)
    ...
    @objc private func deleteTechnician() {
        ...
        showWarning(body: "Are you sure you want to delete this item?", buttonTitle: "Delete") { [weak self] in
            FirestoreDatabaseService.deleteTechnician(deleteItem) { (success) in
                if success {
                    RequestManager.shared.log(date: Date(), object: .technicians, action: .delete, description: deleteItem.description)
                    self?.navigationController?.popViewController(animated: true)
                } else {
                    self?.showAlert("Error", body: "Something went wrong")
                }
            }
        }
    }
...
func showWarning(body: String, buttonTitle: String, action: @escaping () -> Void) {
    let alertController = UIAlertController(title: "Warning", message: body, preferredStyle: .alert)
