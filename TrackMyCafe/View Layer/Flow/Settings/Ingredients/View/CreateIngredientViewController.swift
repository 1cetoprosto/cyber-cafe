...
final class CreateIngredientViewController: UIViewController {
    ...
    // Min Stock Threshold
    private lazy var minStockInputContainer: InputContainerView = {
        let container = InputContainerView(
            labelText: R.string.global.ingredientMinStockThreshold(),
            inputType: .text(keyboardType: UIKeyboardType.decimalPad),
            placeholder: R.string.global.ingredientMinStockThresholdPlaceholder()
        )
        container.enableNumericInput(maxFractionDigits: 3)
        return container
    }()

    private lazy var minStockExplanationLabel: UILabel = {
        let label = AppLabel(style: .footnote)
        label.apply(.footnote)
        label.textColor = UIColor.Main.secondaryText
        label.numberOfLines = 0
        label.text = R.string.global.ingredientMinStockThresholdExplanation()
        return label
    }()
    ...
    private func setupUI() {
        ...
        addSection(input: minStockInputContainer, explanation: minStockExplanationLabel)
    ...
    private func fillData() {
        ...
        if let threshold = ingredient.minStockThreshold {
            minStockInputContainer.text = String(format: "%.3f", threshold)
        } else {
            minStockInputContainer.text = ""
        }
    ...
    private func setupKeyboardHandling() {
        ...
        addDoneButtonToTextField(minStockInputContainer.textFieldReference)
    }
    ...
    @objc private func saveAction() {
        ...
        let thresholdText = (minStockInputContainer.text ?? "").trimmingCharacters(
            in: .whitespacesAndNewlines)
        let minStockThreshold: Double?
        if thresholdText.isEmpty {
            minStockThreshold = nil
        } else {
            guard let parsed = Double(thresholdText.replacingOccurrences(of: ",", with: ".")) else {
                let alert = UIAlertController(
                    title: R.string.global.error(),
                    message: R.string.global
                        .ingredientMinStockThreshold(),
                    preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: "OK", style: .default))
                present(alert, animated: true)
                return
            ...
            guard parsed >= 0 else {
                let alert = UIAlertController(
                    title: R.string.global.error(),
                    message: R.string.global.ingredientMinStockThreshold(),
                    preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: "OK", style: .default))
                present(alert, animated: true)
                return
        ...
        Task {
            if let existingIngredient = ingredientToEdit {
                ...
                updated.unit = unit
                updated.minStockThreshold = minStockThreshold
                await viewModel.updateIngredient(updated)
            } else {
                // Create new
                await viewModel.createIngredient(
                    name: name, cost: cost, stock: stock, unit: unit,
