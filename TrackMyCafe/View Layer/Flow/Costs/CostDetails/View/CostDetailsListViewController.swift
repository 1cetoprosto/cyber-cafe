...
class CostDetailsListViewController: UIViewController {
    ...
    let costNameLabel: UILabel = {
        let label = UILabel(text: "Cost:", font: UIFont.systemFont(ofSize: 20), aligment: .left)

        return label
    }()
    
    let costNameTextfield: UITextField = {
        let textField = UITextField(placeholder: "Enter cost name", font: UIFont.systemFont(ofSize: 28))
    ...
    let costSumTextfield: UITextField = {
        let textField = UITextField(placeholder: "Enter cost sum", font: UIFont.systemFont(ofSize: 28))
...
//    }
    ...
    @objc func saveAction(param: UIButton) {
        viewModel?.saveCostModel(costDate: costdatePiker.date, costName: costNameTextfield.text, costSum: costSumTextfield.text)
...
extension CostDetailsListViewController {
    func setConstraints() {
        ...
        let dateStackView = UIStackView(arrangedSubviews: [costDateLabel, costdatePiker],
                                        axis: .horizontal,
                                        spacing: 20,
                                        distribution: .fill)

        let costStackView = UIStackView(arrangedSubviews: [dateStackView,
                                                               costNameLabel,
                                                               costNameTextfield,
                                                               costSumLabel,
                                                               costSumTextfield,
