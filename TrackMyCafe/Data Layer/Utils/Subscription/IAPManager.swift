...
class IAPManager: NSObject, Loggable {
    ...
    func refreshProStatusUsingStoreKit2() async -> Bool {
        ...
        for await entitlement in StoreKit.Transaction.currentEntitlements {
            ...
            guard let expirationDate = transaction.expirationDate, expirationDate > now else {
                continue
            }
        ...
        if let transaction = bestTransaction, let expirationDate = transaction.expirationDate,
            expirationDate > now
        {
            logger.debug(
                "StoreKit2: Active entitlement found. Product: \(transaction.productID), Exp: \(expirationDate)"
            )
            await MainActor.run {
                self.isProPlan = true
    ...
    func updateInfo(_ subscription: Subscription) {
        ...
        logger.debug(
            "IAPManager: Updating subscription info from RequestManager. Pro: \(subscription.proPlan), Active by Date: \(isActive), Has iOS Sub: \(hasActiveSubscription), Next Payment: \(String(describing: subscription.nextPaymentDate))"
        )
        ...
        if newProStatus {
            ...
            logger.debug(
                "IAPManager: Firebase says not pro, but local state is pro. Keeping local state until receipt verification."
            )
        } else {
            isProPlan = false
        }
    ...
    func getProducts(_ completion: (([SKProduct]?) -> Void)?) {
        ...
        SwiftyStoreKit.retrieveProductsInfo(SubscriptionType.allProductsSet) { (results) in
            let products = Array(results.retrievedProducts).sorted(by: {
        ...
        SwiftyStoreKit.completeTransactions(atomically: true) { purchases in
            for purchase in purchases {
                ...
                case .purchased, .restored:
                    ...
                    self.logger.notice(
                        "\(purchase.transaction.transactionState.debugDescription): \(purchase.productId)"
                    )
    ...
    func verifySubscription(_ completion: ((IAPReceiptAdapterProtocol?) -> Void)? = nil) {
        ...
        if let receiptUrl = Bundle.main.appStoreReceiptURL,
            (try? Data(contentsOf: receiptUrl)) != nil
        {
        ...
        // which fails remote validation on Apple's servers (both Production and Sandbox) with 21002.
            ...
            if ProcessInfo.processInfo.environment["XCODE_RUNNING_FOR_PREVIEWS"] == "1"
                || ProcessInfo.processInfo.arguments.contains("-com.apple.CoreData.SQLDebug")
            {  // Just a heuristic, better to check certificate
        ...
        let appleValidator = AppleReceiptValidator(
            service: .production, sharedSecret: kSharedSecret)
        SwiftyStoreKit.verifyReceipt(using: appleValidator, forceRefresh: false) { (result) in
            ...
            case .error(let error):
                ...
                // We should treat this as a "Success" for local development if we can confirm the purchase happened.
                    ...
                    if isMalformedData {
                        self.logger.error(
                            "Received 21002 in DEBUG. Assuming Xcode StoreKit local receipt. Bypassing remote validation."
                        )
                        ...
                        SwiftyStoreKit.fetchReceipt(forceRefresh: false) { fetchResult in
                            ...
                            case .success(_):
                                ...
                                do {
                                    ...
                                    self.logger.error(
                                        "Local validation failed with error: \(error)")
                                    ...
                                    self.logger.debug(
                                        "Attempting to verify entitlement via active transactions..."
                                    )
                                    ...
                                    let mockTransactionId =
                                        "debug_transaction_\(Int(Date().timeIntervalSince1970))"

                                    self.logger.error(
                                        "Using MOCK Receipt Adapter for DEBUG mode. Product: \(mockProductId)"
                                    )
                                    let mockAdapter = MockIAPReceiptAdapter(
                                        productId: mockProductId, transactionId: mockTransactionId)
                                    // Update subscription status immediately
                ...
                if errorCode == 21007 || errorCode == 21002 {
                    self.logger.debug(
                        "Falling back to Sandbox verification (Error code: \(errorCode))...")
                    let sandboxValidator = AppleReceiptValidator(
                        service: .sandbox, sharedSecret: kSharedSecret)
                    SwiftyStoreKit.verifyReceipt(using: sandboxValidator, forceRefresh: false) {
                        (sandboxResult) in
    ...
    func updateSubscriptionInfo(_ receipt: IAPReceiptAdapterProtocol) {
        ...
        guard let productId = receipt.lastAutorenewProductId,
            ...
            let expireDate = receipt.expireDateForLastAutorenewSubscription
        else {
            ...
            self.logger.debug(
                "TransactionId: \(String(describing: receipt.lastAutorenewTransactionId))")
            self.logger.debug(
                "ExpireDate: \(String(describing: receipt.expireDateForLastAutorenewSubscription))")
            return
        }
